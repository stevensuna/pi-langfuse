...
const SENSITIVE_KEY_PATTERN = /(secret|password|passwd|authorization|cookie|credential|private[_-]?key|api[_-]?key|access[_-]?key|refresh[_-]?token|client[_-]?secret|webhook[_-]?secret)/i;
const TOKEN_KEY_PATTERN = /(^|[_-])(token|tokens)$/i;
const TOKEN_COUNT_KEY_PATTERN = /^(?:(?:max|min|total|prompt|completion|input|output|cache|context|available|remaining|usage|used)[_-]?)*tokens?$/i;
const PUBLIC_KEY_PATTERN = /^public[-_]?key$/i;
const BINARY_KEY_PATTERN = /(image|screenshot|attachment|media|binary|blob|base64|data[_-]?url|file[_-]?bytes|content[_-]?bytes)/i;
...
const ASSIGNMENT_PATTERN = /\b([A-Za-z0-9_-]*(?:SECRET|TOKEN|PASSWORD|PASSWD|AUTHORIZATION|CREDENTIAL|PRIVATE[_-]?KEY|API[_-]?KEY|ACCESS[_-]?KEY|REFRESH[_-]?TOKEN|CLIENT[_-]?SECRET|WEBHOOK[_-]?SECRET)[A-Za-z0-9_-]*)\s*([:=])\s*(["']?)([^\s"'`,}]{6,}|[^\n"']{12,})(\3)/gi;
...
export function isSensitiveKey(key) {
    ...
    if (TOKEN_COUNT_KEY_PATTERN.test(normalized))
        return false;
    return (SENSITIVE_KEY_PATTERN.test(normalized) || TOKEN_KEY_PATTERN.test(normalized));
}
function isBinaryKey(key) {
    return BINARY_KEY_PATTERN.test(normalizeKey(key));
}
function collectExactSecrets(config, env) {
    ...
    for (const [key, value] of Object.entries(env)) {
        if (key === "PI_LANGFUSE_REDACTION_SECRETS")
            continue;
        if (isSensitiveKey(key))
            add(value, key);
    }
    return Array.from(secrets, ([value, reason]) => ({ value, reason })).sort((a, b) => b.value.length - a.value.length);
}
export function redactString(config, input, env = process.env) {
    if (!config.redactionEnabled || !input)
        return input;
    let output = input;
    output = output.replace(ASSIGNMENT_PATTERN, (match, key, operator, quote, value) => isSensitiveKey(key)
    ...
    for (const { reason, pattern } of SECRET_PATTERNS) {
        output = output.replace(pattern, (match) => reason.endsWith("blob") || reason === "data-url"
    ...
    for (const { reason, pattern, validate } of PII_PATTERNS) {
        output = output.replace(pattern, (match) => !validate || validate(match) ? placeholder(reason, match) : match);
    }
    return output;
}
function redactSensitiveField(value, key) {
    if (value === null || value === undefined)
        return value;
    const serialized = typeof value === "string" ? value : JSON.stringify(value);
    return placeholder(key, serialized);
}
function redactBinaryField(value, key) {
    if (value === null || value === undefined)
        return value;
    const serialized = typeof value === "string" ? value : JSON.stringify(value);
    return blobPlaceholder(key, serialized);
}
export function sanitizeForTelemetry(config, value, env = process.env, seen = new WeakSet()) {
    if (!config.redactionEnabled)
        return value;
    if (typeof value === "string")
        return redactString(config, value, env);
    ...
    if (Array.isArray(value)) {
        return value.map((item) => sanitizeForTelemetry(config, item, env, seen));
    }
    const output = {};
...
function addFinding(findings, reason, count = 1) {
    findings.set(reason, (findings.get(reason) ?? 0) + count);
}
export function scanForSecrets(config, input, env = process.env) {
    ...
    for (const secret of collectExactSecrets(config, env)) {
        if (scanInput.includes(secret.value)) {
            addFinding(findings, normalizeReason(secret.reason) || "configured-secret", scanInput.split(secret.value).length - 1);
        }
    }
    const jsonSensitiveFieldPattern = /["']([^"']*(?:secret|password|passwd|authorization|cookie|credential|private[_-]?key|api[_-]?key|access[_-]?key|refresh[_-]?token|client[_-]?secret|webhook[_-]?secret)[^"']*)["']\s*:\s*["']([^"']{6,})["']/gi;
    ...
    for (const { reason, pattern } of SECRET_PATTERNS) {
        ...
        if (matches?.length)
            addFinding(findings, reason, matches.length);
    }
    for (const { reason, pattern, validate } of PII_PATTERNS) {
    ...
    return Array.from(findings, ([reason, count]) => ({ reason, count })).sort((a, b) => a.reason.localeCompare(b.reason));
}
export function redactionMetadata(config) {
