...
export async function ensureLocalLangfuseStarted(config) {
    if (!config.localAutostart)
        return;
    if (process.env.PI_LANGFUSE_AUTOSTART === "0")
        return;
    if (attempted)
        return;
    attempted = true;
    if (!existsSync(`${config.localAutostartDir}/docker-compose.yml`)) {
        warnOnce(`local autostart enabled but no docker-compose.yml found at ${config.localAutostartDir}`);
        return;
    }
    if (await isHealthy(config.localAutostartHealthUrl, config.localAutostartTimeoutMs)) {
        return;
    }
    try {
...
//# sourceMappingURL=local-autostart.js.map